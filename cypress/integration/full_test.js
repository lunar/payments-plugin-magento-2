...
describe('payment plugin full test', () => {
    /**
    ...
    context(`make payments in "${captureModes[0]}" mode`, () => {
        ...
        it(`change plugin capture mode to "${captureModes[0]}"`, () => {
            TestMethods.changePluginCaptureMode(captureModes[0]);
        });
    ...
    context(`make payments in "${captureModes[1]}" mode`, () => {
        /** Modify plugin settings. */
        it(`change plugin capture mode to "${captureModes[1]}"`, () => {
            TestMethods.changePluginCaptureMode(captureModes[1]);
        });
