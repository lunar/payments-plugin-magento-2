    ...
    function (
        ...
        return PaymentDefaultComponent.extend({
            ...
            displayPopup: function () {
                ...
                /** Initialize object. */
                var sdkClient = Paylike({key: window.checkoutConfig.publicapikey});

                var paymentConfig = window.checkoutConfig.config;
                ...
                paymentConfig.amount.value = Math.round(totalAmount * multiplier);
                ...
                sdkClient.pay(paymentConfig, function (err, res) {
                    if (err) {
                        if(err === "closed") {
                            PluginLogger.log("Popup closed by user");
                        }
                        /**
                    ...
                    if (res.transaction.id !== undefined && res.transaction.id !== "") {
                        ...
                        */
                        ...
                        self.messageContainer.addErrorMessage = async function (messageObj) {
                          await PluginLogger.log("Place order failed. Reason: " + messageObj.message);
                        ...
                        */
                        ...
                        self.afterPlaceOrder = async function (args) {
                          await PluginLogger.log("Order placed successfully");
                        ...
                        PluginLogger.log("No transaction id returned from gateway, order not placed");
